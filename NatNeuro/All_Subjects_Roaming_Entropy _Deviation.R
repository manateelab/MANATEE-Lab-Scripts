...
longdata <- longdata[1:nrow(longdata), c("subject.id", "date", "RE")]#Use these columns.
longdata <- longdata %>% rename(Participant_ID= subject.id)
longdata <- longdata %>% rename(RecordedDate= date)
longdata <- longdata %>% rename(RE_Filtered= RE)

#Get participant IDs#
Participant_IDs <- unique(longdata$Participant_ID)

PE_dataframe <- data.frame(matrix(ncol = 5, nrow = nrow(longdata)))
colNames <- c("Participant_ID", "RE_Filtered", "Recorded_Date","Prediction", "PE") #Column names to save.
colnames(PE_dataframe) <- colNames

PE_dataframe$Participant_ID = longdata$Participant_ID;
...
row_counter = 1; #R indexes at 1. Keeps track of current row (will get to 6613).

for(i in 1:length(Participant_IDs)) #Do this for every participant separately.
{	
  inside_counter = 1; #R indexes starting at 1. Start here (the first point of data for this participant).
  while (Participant_IDs[i] == PE_dataframe$Participant_ID[row_counter]) #While on the current participant, run this code.
  {
  ...
  #mergedDataFrame = merge(longdata, PE_dataframe, by.x=c("Participant_ID", "RecordedDate"), by.y=c("ID", "RecordedDate"), all.x = TRUE)
  
}
write.csv(PE_dataframe,file = "Moves_Long_Data_With_PEs.csv", row.names = FALSE)
